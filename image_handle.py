...
#
# # 读取 HDFS 中的图片文件
# hdfs_dir = r"D:\source\python\torch_big_data\Cyclone_Wildfire_Flood_Earthquake_Database\Cyclone"  # 修改为你的 HDFS 路径
...
#
# # 处理并保存图片
# output_base_dir = "./Cyclone"  # 修改为本地保存路径
#
# # 批量处理文件
...
# 读取 HDFS 或本地文件中的图片文件
hdfs_path = r"D:\source\python\torch_big_data\Cyclone_Wildfire_Flood_Earthquake_Database\Wildfire"  # 修改为你的本地路径
image_df = spark.read.format("binaryFile").load(hdfs_path)
...
for batch_start in range(0, file_count, batch_size):
    batch_df = image_df.limit(batch_start + batch_size).collect()[batch_start:]
    output_base_dir = "./Wildfire"  # 修改为本地保存路径
    for row in batch_df:
        file_path = row.path
        image_content = row.content
        filename = os.path.basename(file_path).split(".")[0]
        ...
        # 检查文件是否已经存在
        if os.path.exists(os.path.join(output_dir, filename + ".jpg")):
            print(f"Skipping {filename}, already processed.")
            continue
        processed_images = preprocess_image(image_content, channel_means)
        if processed_images is not None:

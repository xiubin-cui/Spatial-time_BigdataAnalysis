...
def main():
    ...
    try:
        ...
        models = [
            ...
            (
                GBTRegressor(featuresCol="pca_features", labelCol="Ms", maxIter=50, maxDepth=10),
                [],  # 无超参数网格，直接训练
                "GBT Regression",
                False
            )
        ...
        # 训练并评估模型
        for model, param_grid, name, use_cv in models:
            rmse, accuracy, _ = train_and_evaluate_model(
                model, param_grid, training_data, test_data, use_cv=use_cv
            )

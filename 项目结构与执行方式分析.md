...
- config.py：配置文件，管理路径和常量。
- utils.py：工具模块，提供文件操作、图像处理、消息框等功能。
- main.py：主程序入口，启动应用程序。
- Main_window_UI_interface.py：主窗口 UI 界面（由 untitled.ui 编译）。
- Main_window_logic.py：主窗口逻辑实现，包含文件操作和可视化功能。
- Data_Visualization_UI.py：数据可视化窗口 UI 界面（由 data_view.ui 编译）。
- Confusion_matrix_heat_map.py：混淆矩阵热力图生成模块。
- untitled.ui 和 data_view.ui：UI 设计文件（无需修改，但需确保与 Python 文件同步）。
